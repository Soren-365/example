...
const notesFetch = async () => {
  ...
  const { data: note, error }: PostgrestSingleResponse<INote[]> = await supabase
    .from('note')
    .select('*');
  ...
  const string = JSON.stringify({ note }, null, 2);
  return note;
};
                  ...
                  <div className="mt-2 sm:flex sm:justify-between">
                    ...
                    <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                      ...
                      <p>
                        ...
                        <time dateTime={new Date(note.createdat).toDateString()}>{note.createdat}</time>
                      </p>
                    </div>
                  </div>
                </div>
